...
import { WishlistItem } from '@/lib/discogs';
...
export function WishlistView({ items: initialItems }: Props) {
  ...
  const getOriginalYear = (item: WishlistItem): number => {
  ...
  const handleAddToCollection = async (releaseId: number) => {
    ...
    try {
      ...
      // Auto-remove from wishlist after successful add to collection
      ...
      if (!removeRes.ok) {
        throw new Error('Added to collection, but failed to remove from wishlist');
      }
      ...
      console.error('Failed to add/remove wishlist item:', error);
    }
    setAdding(null);
  };
          ...
          <div className="bg-zinc-900 rounded-xl border border-zinc-700 max-w-md w-full p-6">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-white mb-1">Remove from Wishlist?</h3>
...
function WishlistCard({ item, onOpenDetails, onRemove, onAddToCollection, isRemoving, isAdding, extras }: {
  item: WishlistItem;
  onOpenDetails: () => void;
  onRemove: () => void;
